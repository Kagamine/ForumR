...
namespace ForumR.Controllers
{
    public class ForumController : BaseController
    {
        ...
        public IActionResult Post(long id, Guid? pid, string content)
        {
            ...
            if (pid.HasValue)
            {
                ForumHub.Clients.Group("Thread-" + thread.Id).OnPostChanged(p.ParentId);
                return Content(p.ParentId.ToString());
            }
            else
            {
                ForumHub.Clients.Group("Thread-" + thread.Id).OnPostChanged(p.Id);
        ...
        public IActionResult Open(string id, string Title, string Content)
        {
            ...
            forum.ThreadCount++;
            DB.SaveChanges();
            ForumHub.Clients.Group("Forum-" + thread.ForumId).OnThreadChanged(thread.Id);
        ...
        public string RemovePost(Guid id)
        {
            ...
            DB.SaveChanges();
            ForumHub.Clients.Group("Thread-" + post.ThreadId).OnPostRemoved(post.Id);
            return "回复已成功删除";
        }
        ...
        public IActionResult EditPost(long id, Guid pid, string content)
        {
            ...
            DB.SaveChanges();
            ForumHub.Clients.Group("Thread-" + thread.Id).OnPostChanged(post.Id);
            return Content(Marked.Parse(content));
        }
